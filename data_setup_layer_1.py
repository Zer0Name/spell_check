...
if choice == "hash":
	...
	F = open('last_state_name_layer1.txt','w')
	F.write(name)
	F.close()
else:
	if choice != "y":

		F = open('last_state_name_layer1.txt','r')
		for line in F:
			name = line.strip()
		F.close()
...
model = Sequential()
model.add(LSTM(256, input_shape=(X.shape[1], X.shape[2]) , name = "first_layer"))
model.add(Dense(y.shape[1], activation='softmax',name = name ) )
try:
	model.load_weights("weights_layer1.hdf5", by_name=True) 
except:
	print "file not there"
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

filepath="weights_layer1.hdf5"
